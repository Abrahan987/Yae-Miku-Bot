...
const rojo = '\x1b[38;2;255;100;100m';
const reset = '\x1b[0m';
...
const limpiarSesion = () => {
    if (fs.existsSync(sDir)) {
        try {
            ...
            console.error(`${rojo}[ ERROR ] No se pudo limpiar la sesión:${reset}`, err);
        }
    }
};
...
`);
        ...
        if (usarCodigo && !state.creds.registered) {
            ...
            setTimeout(async () => {
                try {
                    ...
                    console.log(
                        `🪷 𝗧𝘂 𝗰𝗼́𝗱𝗶𝗴𝗼 𝗲𝘀: ${rosaLila}${subrayado}${code}${reset}`
                    );
                } catch (err) {
                    console.error(
                        `${rojo}[ ERROR ] Falla al generar código de vinculación:${reset}`,
                        err
                    );
        ...
        sock.ev.on('creds.update', async () => {
            try {
                ...
                console.error(
                    `${rojo}[ ERROR ] Error guardando credenciales:${reset}`,
                    err
                );
            }
        });
            ...
            try {
                ...
                if (qr && esQR) {
                    try {
                        ...
                        console.error(
                            `${rojo}[ ERROR ] Error al renderizar el código QR:${reset}`,
                            err
                        );
                    }
                }
                ...
                if (connection === 'close') {
                    ...
                    if (statusCode === 515 || isStreamError) {
                        console.log(
                            `${lila}[ RECONNECT ] Reiniciando socket por actualización de sesión...${reset}`
                        );

                        setTimeout(() => {
                            startBot().catch((error) => {
                                console.error(
                                    `${rojo}[ ERROR RECONNECT ]${reset}`,
                                    error
                                );
                            });
                        }, 2000);
                        ...
                        console.log(
                            `${rosa}[ SESSION ] Sesión cerrada desde el teléfono. Limpiando...${reset}`
                        );
                        ...
                        console.log(
                            `${lila}[ CONEXION ] Desconectado (Status ${statusCode}). Reintentando...${reset}`
                        );
                ...
                console.error(
                    `${rojo}[ ERROR CONNECTION.UPDATE ]${reset}`,
                    err
                );
            }
        });
        ...
        try {
            ...
            console.error(
                `${rojo}[ ERROR HANDLER ] El handler produjo un error:${reset}`
            );
        ...
        console.error(
            `${rojo}[ ERROR STARTBOT ] Error iniciando el bot:${reset}`
        );
...
process.on('unhandledRejection', (reason) => {
    console.error(
        `\n${rojo}[ UNHANDLED REJECTION ]${reset}`
    );
...
startBot().catch((err) => {
    console.error(
        `\n${rojo}[ FATAL ] El bot no pudo iniciar:${reset}`
    );
